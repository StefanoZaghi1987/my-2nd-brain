...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 vault.config.yml has reflect_reminder_days: 14 in the lint: section
- [x] #2 vault_state.py _DEFAULTS["lint"] includes "reflect_reminder_days": 14
- [x] #3 TestLoadConfig.test_returns_all_default_sections_when_absent asserts config["lint"]["reflect_reminder_days"] == 14
- [x] #4 pytest tests/test_vault_state.py passes with no regressions
<!-- AC:END -->
...
Added `reflect_reminder_days: 14` to `_DEFAULTS["lint"]` in vault_state.py and to the `lint:` section of vault.config.yml. Added the corresponding assertion to `TestLoadConfig.test_returns_all_default_sections_when_absent`. All 14 vault_state tests pass. Commit: 463b897.
